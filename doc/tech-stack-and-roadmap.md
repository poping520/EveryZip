...
`IArchiveParser` 提供统一接口，当前主线使用 `ZipArchiveParser`。这个抽象仍然保留给后续多解析器调度使用，但候选解析方案必须能在列表模式下提供条目的压缩后大小。

当前现实状态：

- ZIP/APK：已实际走 minizip。
- 7z、rar、tar、tar.gz 等：仍是长期目标，但不再采用 libarchive，需要评估其他解析库或格式专用解析器。
...
- 逐步支持 7z、tar、tar.gz、gz 等格式，并单独评估 rar 的授权和依赖约束。
