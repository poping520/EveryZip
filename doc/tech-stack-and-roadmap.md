...
EveryZip 是一个 Windows 桌面工具，目标是用接近 Everything 的体验，快速检索本机归档文件内部条目。当前实现重点是：快速发现本机 ZIP/7z/RAR 等归档文件，解析内部文件列表，写入本地 SQLite 索引，并在 Win32 ListView 中低延迟检索和浏览。
...
- 当前主要表：
  - `archives`：归档文件元数据，包括盘符、文件引用号、USN、路径、大小、修改时间。
...
- zlib + minizip：用于 ZIP 条目枚举和单条目解压。
...
- `ZipArchiveParser`：当前已接入主程序的 ZIP 解析器。
...
- 默认归档扩展名：`.zip`、`.7z`、`.rar`。
- 测试阶段默认扫描盘符：`G`；`scan_drives = []` 表示扫描所有 NTFS 盘。
...
`IArchiveParser` 提供统一接口，当前主线按扩展名分发到 ZIP、7z、RAR 专用解析器。这个抽象仍然保留给后续多解析器调度使用，但候选解析方案必须能在列表模式下提供条目的压缩后大小。
...
- 当前主线实际支持 ZIP/7z/RAR，其中 7z 的 solid archive 条目没有可靠逐文件压缩大小，界面显示 `-`。
