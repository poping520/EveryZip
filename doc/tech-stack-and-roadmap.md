...
EveryZip 是一个 Windows 桌面工具，目标是用接近 Everything 的体验，快速检索本机归档文件内部条目。当前实现重点是：快速发现本机 ZIP/APK/7z 等归档文件，解析内部文件列表，写入本地 SQLite 索引，并在 Win32 ListView 中低延迟检索和浏览。
...
- rar、tar、tar.gz 等：仍是长期目标，但不再采用 libarchive，需要评估其他解析库或格式专用解析器。
...
- 逐步支持 tar、tar.gz、gz 等格式，并单独评估 rar 的授权和依赖约束。
...
- 当前主线实际支持 ZIP/APK/7z，其中 7z 的 solid archive 条目没有可靠逐文件压缩大小，界面显示 `-`。
