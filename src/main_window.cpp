...
static constexpr wchar_t kAboutClass[] = L"EveryZipAboutWindow";
...
static constexpr int IDC_ABOUT_LINK_CTRL = 3040;
...
static void ShowAboutPanel(HWND hOwner, MainWindowState* s);
...
static bool IsEnglishUi(MainWindowState* s) {
    return s && LS_(s, IDS_ABOUT_TITLE) == L"About";
}

static std::wstring GetAboutVersionText(MainWindowState* s) {
    return IsEnglishUi(s) ? L"Version 1.0.0" : L"版本 1.0.0";
}

static std::wstring GetAboutSummaryText(MainWindowState* s) {
    return IsEnglishUi(s)
        ? L"Lightweight archive indexing and search tool"
        : L"轻量的压缩文件索引与检索工具";
}

static std::wstring GetAboutProjectUrl() {
...
static LRESULT CALLBACK AboutWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    AboutWindowState* aws = (AboutWindowState*)GetWindowLongPtrW(hWnd, GWLP_USERDATA);
    ...
    if (msg == WM_CREATE) {
        ...
        const UINT dpi = GetWindowDpi(hWnd);
        const int margin = ScaleDpi(20, dpi);
        const int iconSize = ScaleDpi(48, dpi);
        const int contentTop = margin;
        const int contentLeft = margin + iconSize + ScaleDpi(16, dpi);
        const int contentWidth = ScaleDpi(236, dpi);
        const int lineH = ScaleDpi(22, dpi);
        const int titleH = ScaleDpi(30, dpi);
        const int buttonW = ScaleDpi(88, dpi);
        const int buttonH = ScaleDpi(28, dpi);
        const int buttonY = ScaleDpi(166, dpi);

        HWND hIcon = CreateWindowExW(0, L"STATIC", nullptr,
            WS_CHILD | WS_VISIBLE | SS_ICON,
            margin, contentTop + ScaleDpi(2, dpi), iconSize, iconSize,
            hWnd, nullptr, s ? s->hInstance : nullptr, nullptr);

        HWND hTitle = CreateWindowExW(0, L"STATIC",
            s ? LS_(s, IDS_ABOUT_TEXT).c_str() : L"EveryZip",
            WS_CHILD | WS_VISIBLE,
            contentLeft, contentTop, contentWidth, titleH,
            hWnd, nullptr, s ? s->hInstance : nullptr, nullptr);

        HWND hVersion = CreateWindowExW(0, L"STATIC",
            GetAboutVersionText(s).c_str(),
            WS_CHILD | WS_VISIBLE,
            contentLeft, contentTop + titleH, contentWidth, lineH,
            hWnd, nullptr, s ? s->hInstance : nullptr, nullptr);

        HWND hSummary = CreateWindowExW(0, L"STATIC",
            GetAboutSummaryText(s).c_str(),
            WS_CHILD | WS_VISIBLE,
            margin, contentTop + iconSize + ScaleDpi(16, dpi), ScaleDpi(300, dpi), lineH,
            hWnd, nullptr, s ? s->hInstance : nullptr, nullptr);
        ...
        HWND hLink = CreateWindowExW(0, L"SysLink", linkText.c_str(),
            ...
            margin, contentTop + iconSize + ScaleDpi(44, dpi), ScaleDpi(300, dpi), lineH,
            hWnd, (HMENU)(INT_PTR)IDC_ABOUT_LINK_CTRL, s ? s->hInstance : nullptr, nullptr);

        HWND hOk = CreateWindowExW(0, L"BUTTON",
            s ? LS_(s, IDS_SETTINGS_OK).c_str() : L"OK",
            WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON,
            (ScaleDpi(336, dpi) - buttonW) / 2, buttonY, buttonW, buttonH,
            hWnd, (HMENU)(INT_PTR)IDOK, s ? s->hInstance : nullptr, nullptr);
        ...
        if (SystemParametersInfoW(SPI_GETNONCLIENTMETRICS, sizeof(ncm), &ncm, 0)) {
            ...
            titleLf.lfHeight = -ScaleDpi(18, dpi);
    ...
    case WM_DESTROY:
        if (aws) {
            if (aws->hFont) {
                DeleteObject(aws->hFont);
                aws->hFont = nullptr;
            }
            if (aws->hTitleFont) {
                DeleteObject(aws->hTitleFont);
                aws->hTitleFont = nullptr;
            }
            if (aws->hIcon) {
                DestroyIcon(aws->hIcon);
                aws->hIcon = nullptr;
            }
        }
        return 0;
...
static void RegisterAboutClass(HINSTANCE hInstance) {
    static bool registered = false;
    if (registered) return;
...
static void ShowAboutPanel(HWND hOwner, MainWindowState* s) {
    ...
    const UINT dpi = GetWindowDpi(hOwner);
    const int width = ScaleDpi(336, dpi);
    const int height = ScaleDpi(242, dpi);
    ...
    while (IsWindow(hDlg)) {
        ...
        if (msg.message == WM_KEYDOWN && msg.wParam == VK_RETURN) {
            HWND target = msg.hwnd;
            if ((target == hDlg || IsChild(hDlg, target)) && target != GetDlgItem(hDlg, IDC_ABOUT_LINK_CTRL)) {
