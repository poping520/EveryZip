...
EveryZip 是一个 Windows 桌面工具，目标是用接近 Everything 的体验，快速检索本机归档文件内部条目。当前实现重点是：快速发现本机 ZIP/APK 等归档文件，解析内部文件列表，写入本地 SQLite 索引，并在 Win32 ListView 中低延迟检索和浏览。
...
- zlib + minizip：当前主线解析器，实际用于 ZIP/APK 条目枚举和单条目解压。
- 解析接口抽象：`IArchiveParser` 定义 `Open`、`ListEntries`、`ExtractEntry` 等通用能力。
- `ZipArchiveParser`：当前已接入主程序的实际解析器。
- `LibArchiveParser`：代码中已有雏形，可支持多格式归档的统一接口，但当前 CMake 中未启用，主线也未切换到它。
...
`IArchiveParser` 提供统一接口，当前主线使用 `ZipArchiveParser`。这为后续切换到 libarchive 或多解析器调度留下了扩展点。

当前现实状态：

- ZIP/APK：已实际走 minizip。
- 7z、rar、tar、tar.gz 等：有 libarchive 方向的代码草稿，但未纳入构建和运行路径。
...
- 正式引入 libarchive 构建和运行依赖。
- 实现按扩展名或探测结果选择解析器。
- 支持 7z、tar、tar.gz、gz 等格式。
- 明确 rar 支持策略：取决于 libarchive 构建能力和授权/依赖约束。
