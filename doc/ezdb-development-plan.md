...
- 当前 v5 以“磁盘空间优先 + 运行时内存优化”为目标，在 v4 目录树、compact records 与自适应 postings container 基础上增加独立 section/container 压缩，并把打开后的文件记录改为列式内存结构。
...
当前 v5 使用 compact file records 和目录记录两个 section。磁盘上 file records、dir records、string pool 可以独立 zlib 压缩；打开数据库时 file records 通过 zlib 流式解码直接写入列式内存数组，目录数组和字符串池仍一次性解压加载，保证 `get/search` 可以按 id 快速访问。
...
- 当前 v5 还没有真实持久化增删改。
- v5 不兼容 v1/v2/v3/v4 `.ezdb` 文件，旧文件需要重新 build。
