...
- 当前 v5 以“磁盘空间优先”为目标，在 v4 目录树、compact records 与自适应 postings container 基础上增加独立 section/container 压缩，进一步减少 `.ezdb` 文件体积。
...
- magic：固定为 `EZDB0005`，用于识别文件类型。
- version：格式版本号。v5 不兼容 v1/v2/v3/v4，旧 `.ezdb` 文件需要重新构建。
- file_count：文件记录总数。
- active_count：未删除记录数。
- dir_count：目录节点数量。
...
当前 v5 使用 compact file records 和目录记录两个 section。磁盘上 file records、dir records、string pool 可以独立 zlib 压缩；打开数据库时解压到内存中的轻量文件记录数组、目录数组和字符串池，保证 `get/search` 可以按 id 快速访问。
...
- 打开数据库时一次读取 compact records section 并在内存中解码，避免逐字节 `fgetc` 导致 open 变慢。
- 内存结构仍保留 `parent_dir_id + name_offset + name_len + size + modified_time`，查询代码改动较小。

缺点：

- 当前为了热查询速度，打开后内存仍解码为结构数组，常驻内存还可以继续优化。
- records/dirs/names 压缩会增加 cold open 时间和打开阶段峰值内存，但不影响热查询路径。
- size/mtime 尚未使用 block base-delta，后续仍有进一步压缩空间。
...
- 查询仍按需读取 postings；records/dirs/names 在 open 阶段一次性解压到内存，换取热查询阶段无需反复解压。
...
- compact file records 解码后的内存记录区。
- index gram 映射表。
- 文件记录表。
- 目录记录表。
- 字符串池。
...
1. file records 改为真正列式内存结构，降低 open 后常驻内存。
2. size/mtime 使用 block base-delta，继续压缩 records section。
3. 对字符串池增加块压缩。
4. postings builder 改为更连续的 arena/block 存储，进一步降低构建时间。
...
| 关键词 | open_ms | search_ms | returned | private MB |
| --- | ---: | ---: | ---: | ---: |
| `a` limit=20 | 412 | 21 | 20 | 233.88 |
| `aa` | 419 | 1 | 20 | 232.90 |
| `aaa` | 399 | 0 | 20 | 232.90 |
| `aaaa` | 395 | 1 | 20 | 232.91 |
| `index.js` | 413 | 18 | 20 | 234.44 |
| `.dll` | 405 | 1 | 20 | 233.22 |
| `metadata.bin` | 405 | 6 | 20 | 233.57 |
| `node_modules` | 404 | 5 | 20 | 232.90 |
| `Program Files` | 403 | 4 | 20 | 232.98 |
| `设计` | 415 | 3 | 3 | 232.89 |
| `方案` | 420 | 3 | 18 | 232.88 |
| `zzznotfoundzzz` | 397 | 3 | 0 | 232.90 |
| `a` limit=0 | 393 | 610 | 5,238,437 | 234.23 |
...
- 代价是 cold open 从约 100ms 增加到约 400ms，打开阶段峰值内存约 319 MB。后续如果更重视打开速度，可考虑只压 postings 和 names，或把 records/dirs 改成分块懒解压。
...
- 当前 v3 还没有真实持久化增删改。
- v4 不兼容 v1/v2/v3 `.ezdb` 文件，旧文件需要重新 build。
- 文件记录已做磁盘压缩，但内存中仍解码为结构数组，常驻内存仍有优化空间。
- 目录索引基于路径组件，不为任意跨组件子串单独建索引。
- 高频 1 字节关键词可能返回接近全库，耗时主要由候选确认和结果回调决定。
- 没有崩溃恢复、校验和、compact、并发读写控制。
- 530 万 v4 Release 基准已完成；后续重点是构建时间优化和常驻内存优化。
