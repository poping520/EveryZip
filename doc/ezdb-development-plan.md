...
- 当前 v4 以“均衡优化”为目标，在 v3 目录树和组件级索引基础上增加 compact records 与自适应 postings container，继续减少文件体积和构建峰值内存。
...
- magic：固定为 `EZDB0004`，用于识别文件类型。
- version：格式版本号。v4 不兼容 v1/v2/v3，旧 `.ezdb` 文件需要重新构建。
- file_count：文件记录总数。
- active_count：未删除记录数。
- dir_count：目录节点数量。
...
当前 v4 使用 compact file records 和固定长度目录记录两个 section。内存打开后仍解码为轻量文件记录数组，保证 `get/search` 可以按 id 快速访问。
...
- 当前只是磁盘压缩，打开后内存仍解码为结构数组，常驻内存还可以继续优化。
...
| 构建耗时 | 66,941 ms / 66.94s | 74,002 ms / 74.00s |
...
- 构建时间从 66.94s 变为 74.00s，未达到本轮构建加速目标；主要原因是当前 postings builder 使用大量独立 id list 聚合，降低内存和体积但增加了分散内存访问成本。下一轮应把 builder 改为连续 arena 或真正分桶外排。
