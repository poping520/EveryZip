...
- 当前 v6 在 v5 的“磁盘空间优先 + 运行时内存优化”基础上增加 append-only delta log，实现持久化 `insert/update/delete`，同时保持压缩基座不可变。
...
- magic：固定为 `EZDB0006`，用于识别文件类型。
- version：格式版本号。v6 不兼容 v1/v2/v3/v4/v5，旧 `.ezdb` 文件需要重新构建。
...
当前 v6 沿用 v5 的 compact file records 和目录记录两个 section。磁盘上 file records、dir records、string pool 可以独立 zlib 压缩；打开数据库时 file records 通过 zlib 流式解码直接写入列式内存数组，目录数组和字符串池仍一次性解压加载，保证 `get/search` 可以按 id 快速访问。
...
- `insert/update/delete` 成功返回前已经追加 delta log 并刷新到磁盘；只读打开时返回 `EZDB_ERR_READ_ONLY`。
...
- v6 不兼容 v1/v2/v3/v4/v5 `.ezdb` 文件，旧文件需要重新 build。
- 文件记录已做磁盘压缩和运行时列式常驻，但目录记录、字符串池和索引仍在 open 后整体常驻。
- 目录索引基于路径组件，不为任意跨组件子串单独建索引。
- 高频 1 字节关键词可能返回接近全库，耗时主要由候选确认和结果回调决定。
- 当前 delta log 已按“先追加并 flush，再更新 header 并 flush”的顺序写入，但还没有校验和、事务提交标记、自动截断不完整 delta、compact、并发读写控制。
- delta 覆盖层当前面向小批量在线写入；大量增删改后需要 compact 或独立 delta 索引避免 open replay 和 delta 搜索扫描增长。
- 530 万 v6 Release 基准已完成；build 峰值内存保持在当前电脑约 1.24 GB。后续重点是 compact、崩溃恢复、进一步降低字符串池/索引常驻内存，并优化高频短词 `limit > 0` 的候选展开。
